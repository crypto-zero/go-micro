...
func Sign(CACrt, CAKey, CSR []byte, opts ...CertOption) ([]byte, error) {
	...
	if len(asn1CACrt) != 1 {
		return nil, fmt.Errorf("expected 1 CA Crt, got %d: %w", len(asn1CACrt), err)
	}
	...
	if len(asn1CAKey) != 1 {
		return nil, fmt.Errorf("expected 1 CA Key, got %d: %w", len(asn1CACrt), err)
	}
	...
	if len(asn1CSR) != 1 {
		return nil, fmt.Errorf("expected 1 CSR, got %d: %w", len(asn1CSR), err)
	}
